...
export const SuperAdminPanel: React.FC<SuperAdminPanelProps> = ({ onClose }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (pinInput === config.superAdminPin || pinInput === '6655') {
      ...
      setPinError('Invalid PIN. Master PIN is 6655.');
    }
  };
  ...
  const handleUpdateDishSubmit = (e: React.FormEvent) => {
    ...
    const updated = config.dishes.map((d) => (d.id === editingDish.id ? editingDish : d));
    const updatedConfig = { ...config, dishes: updated };
    setConfig(updatedConfig);
    saveSiteConfig(updatedConfig);
    setEditingDish(null);
    setSaveToast(`Dish "${editingDish.name}" updated successfully!`);
    setTimeout(() => setSaveToast(''), 4000);
  };
  ...
  const handleCreateNewDish = (e: React.FormEvent) => {
    ...
    const created: DynamicDish = {
      ...
      imageUrl: newDishImg.trim() || 'https://images.unsplash.com/photo-1546833999-b9f581a1996d?w=800&auto=format&fit=crop&q=80',
      badge: newDishBadge.trim() || undefined,
      isAvailable: true,
    };

    const updated = [...config.dishes, created];
    const updatedConfig = { ...config, dishes: updated };
    setConfig(updatedConfig);
    saveSiteConfig(updatedConfig);
    ...
    setSaveToast(`Dish "${created.name}" added!`);
    setTimeout(() => setSaveToast(''), 4000);
  };
      ...
      <div className="min-h-screen bg-[#111A14] flex items-center justify-center p-4 text-[#FAF7F2]">
        <div className="max-w-md w-full bg-[#18261E] border border-[#D97706]/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          <div className="text-center mb-8">
            ...
            <p className="text-emerald-300/70 text-xs mt-1">Master PIN: 6655</p>
          </div>
    ...
    <div className="min-h-screen bg-[#0E1712] text-[#FAF7F2] flex flex-col font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-emerald-300/60 text-xs">Dynamic Menu, Edit Dishes & Photos, Accounts</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex-1 w-full">
                  ...
                  >
                    <div>
                      ...
                      <div className="relative h-44 w-full rounded-2xl overflow-hidden mb-3 bg-[#18271E]">
                        ...
                        {dish.badge && (
                          <span className="absolute top-2.5 right-2.5 px-2.5 py-0.5 rounded-lg text-[10px] font-black bg-amber-500 text-slate-950 shadow">
                            {dish.badge}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="px-3 py-1.5 bg-[#203326] hover:bg-[#2c4734] text-amber-300 border border-amber-500/30 rounded-xl text-xs font-bold transition cursor-pointer flex items-center gap-1"
                          >
                            <span>✏️</span> Edit
                          </button>
                          ...
                          <button
                            ...
                            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold cursor-pointer transition ${
                          ...
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-rose-600/20 hover:bg-rose-600 text-rose-300 hover:text-white rounded-xl text-xs font-bold cursor-pointer transition"
                            title="Delete"
                          >
                            🗑️
                          </button>
              ...
              <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-4">
                <div className="relative max-w-lg w-full bg-[#15231B] border-2 border-amber-500/50 rounded-3xl p-6 text-white space-y-4 shadow-2xl">
                  ...
                  <form onSubmit={handleUpdateDishSubmit} className="space-y-3.5">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-300 mb-1">Photo Image URL *</label>
                      <input
                        type="text"
                        required
                  ...
                  <form onSubmit={handleCreateNewDish} className="space-y-3">
                    ...
                    <div>
                      ...
                      <textarea
                        rows={2}
                        required
                        placeholder="e.g. 4 Rotis + Sabzi + Dal + Rice"
                        value={newDishItems}
        ...
        {activeTab === 'banking' && (
          <form onSubmit={handleSave} className="space-y-6">
            <div className="bg-[#15231B] border border-[#243B2D] rounded-3xl p-6 sm:p-8 space-y-6">
              <h3 className="text-base font-black text-amber-300">🏦 Banking & UPI QR</h3>
